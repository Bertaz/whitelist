...
${GRAVITY_UPDATE_COMMAND} $(xargs < "${PIHOLE_LOCATION}"/whitelist.txt) --comment "AnudeepND/Whitelist" #> /dev/null
...
sleep 0.1
echo -e " ${TICK} \e[32m Removing duplicates... \e[0m"
mv "${PIHOLE_LOCATION}"/whitelist.txt "${PIHOLE_LOCATION}"/whitelist.txt.old && sort -u "${PIHOLE_LOCATION}"/whitelist.txt.old | sed -n '/^\s*$/d; /^#/!p' > "${PIHOLE_LOCATION}"/whitelist.txt
rm "${PIHOLE_LOCATION}"/whitelist.txt.old

echo -e " [...] \e[32m Pi-hole gravity rebuilding lists. This may take a while... \e[0m"
${GRAVITY_UPDATE_COMMAND} $(xargs < "${PIHOLE_LOCATION}"/whitelist.txt) --comment "AnudeepND/Optional" #> /dev/null
rm "${PIHOLE_LOCATION}"/whitelist.txt
